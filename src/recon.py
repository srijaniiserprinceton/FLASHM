...
def MP5(x, phi_master, v, N_ghost, alpha):
    ...
    for j in [2, 3]:
        ...
        phi_jph_orig = (2.0 * phi[N_ghost - 3 + j:-N_ghost - 3 + j]
                        - 13.0 * phi_master[j - 1]
                        + 47.0 * phi[N_ghost - 1 + j:-N_ghost - 1 + j]
                        + 27 * phi[N_ghost + j: -N_ghost + j]
                        - 3.0 * phi[N_ghost + 1 + j:-N_ghost + 1 + j]) / 60.0  # for right-going wind

        phi_MP = phi[N_ghost - 1 + j:-N_ghost - 1 + j] \
                 + minmod(np.array([phi[N_ghost + j: -N_ghost + j] - phi[N_ghost - 1 + j:-N_ghost - 1 + j],
                                    alpha * (phi[N_ghost - 1 + j:-N_ghost - 1 + j]
                                             - phi_master[j - 1])]))

        # creating the array to be compared with epsilon at each interface
        cond_array = np.multiply(phi_jph_orig - phi[N_ghost - 1 + j:-N_ghost - 1 + j],
                                 phi_jph_orig - phi_MP)
        ...
        # further computation if the condition is not satisfied
        d = phi_master[j - 1] + phi[N_ghost + j: -N_ghost + j] - 2 * phi[N_ghost - 1 + j:-N_ghost - 1 + j]
        dp1 = phi[N_ghost - 1 + j:-N_ghost - 1 + j] + phi[N_ghost + 1 + j:-N_ghost + 1 + j] - 2 * phi[N_ghost + j: -N_ghost + j]
        dm1 = phi[N_ghost - 3 + j:-N_ghost - 3 + j] + phi[N_ghost - 1 + j:-N_ghost - 1 + j] - 2 * phi_master[j - 1]
        ...
        phi_UL = phi[N_ghost - 1 + j:-N_ghost - 1 + j] + alpha * (
                phi[N_ghost - 1 + j:-N_ghost - 1 + j] - phi_master[j - 1])

        phi_AV = 0.5 * (phi[N_ghost - 1 + j:-N_ghost - 1 + j] + phi[N_ghost + j: -N_ghost + j])

        phi_MD = phi_AV - 0.5 * d_M4_jph

        phi_LC = phi + 0.5 * (phi[N_ghost - 1 + j:-N_ghost - 1 + j] - phi_master[j - 1]) + (
                4.0 / 3.0) * d_M4_jmh

        phi_min = np.maximum(
            np.minimum(phi[N_ghost - 1 + j:-N_ghost - 1 + j], phi[N_ghost + j: -N_ghost + j], phi_MD),
            np.minimum(phi[N_ghost - 1 + j:-N_ghost - 1 + j], phi_UL, phi_LC))
        phi_max = np.minimum(
            np.maximum(phi[N_ghost - 1 + j:-N_ghost - 1 + j], phi[N_ghost + j: -N_ghost + j], phi_MD),
            np.maximum(phi[N_ghost - 1 + j:-N_ghost - 1 + j], phi_UL, phi_LC))
